...
const TAB_LABELS = { records: 'historyTabRecords', numbers: 'numbers', backup: 'historyTabBackup' }
...
export default function HistoryView({
  ...
  // History piles up records, the numbers computed from them and the backup
  // tools all in one place -- tabs keep those from competing for the same
  // screen, since only one is what you came here for at a time.
  ...
  return (
    <>
          ...
          {sorted.length === 0 ? (
            ...
            years.map((group) => (
              <section className="section" key={group.year}>
                ...
                <div className="show-list show-list--compact">
      ...
      {tab === 'numbers' && <StatsView events={allEvents} />}

      {tab === 'backup' && (
        <>
          {/* A restore belongs where the records are: this is the screen you
              open after reinstalling, when Events is empty and nothing looks
              familiar. Export covers every category, not just the past
              events. */}
